...
from bert import bert_encode_text, get_encoded_dim, DEFAULT_BERT_NAME
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # relative positional encoding (bert style)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        *,
        dim,
        fusion_embed_dim = get_encoded_dim(DEFAULT_BERT_NAME),
        num_resnet_blocks = 1,
        cond_dim = None,
        num_image_tokens = 4,
        num_time_tokens = 2,
       ...
       # print(cond_dim)
        if cond_on_text:
            assert exists(fusion_embed_dim), 'fusion_embed_dim must be given to the unet if cond_on_text is True'
            self.text_to_cond = nn.Linear(fusion_embed_dim, cond_dim)
        ...
        self.null_fusion_embed = nn.Parameter(torch.randn(1, max_text_len, cond_dim))
    ...
    def cast_model_parameters(
        ...
        *,
        ...
        if lowres_cond == self.lowres_cond and \
            ...
            fusion_embed_dim == self._locals['fusion_embed_dim'] and \
    ...
    def forward(
        ...
        *,
        ...
        if exists(text_embeds) and self.cond_on_text:
            ...
            conditional_tokens = self.text_to_cond(text_embeds)
         #   print(self.max_text_len, "conditional_tokens")
            conditional_tokens = conditional_tokens[:, :self.max_text_len]
           # print(conditional_tokens.shape)
           # print(text_mask)
            text_mask_batch_size = text_embeds.shape[0]
            #text_mask = text_mask.view(text_mask_batch_size, 1)
            if exists(text_mask):
                text_mask = text_mask[:, :self.max_text_len]
           # print(conditional_tokens.shape, "---->")
            conditional_tokens_len = conditional_tokens.shape[1]
            remainder = self.max_text_len - conditional_tokens_len
            ...
            null_fusion_embed = self.null_fusion_embed.to(conditional_tokens.dtype) # for some reason pytorch AMP not working
           # print(conditional_tokens.shape, "---->")
            ...
            text_hiddens = self.to_text_non_attn_cond(mean_pooled_conditional_tokens)
